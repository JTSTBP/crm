...
const AddPOCModal: React.FC<AddPOCModalProps> = ({ isOpen, onClose, leadId, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/api/leads/${leadId}/poc`, {
                method: 'POST',
                headers: {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between p-6 border-b border-slate-100 bg-slate-50/50">
                    <div>
                        <h2 className="text-xl font-extrabold text-[#0f1c2e]">Add New Contact</h2>
                        <p className="text-xs font-bold text-slate-400 mt-0.5">Enter details for the new point of contact</p>
                    </div>
