...
import {
    Download, PieChart as PieChartIcon, TrendingUp, Briefcase,
    PhoneCall, Loader2, Users, Award, UserCheck,
    Mail, Phone, Calendar
} from 'lucide-react';
...
interface ReportData {
    ...
    managerInfo: { name: string; role: string } | null;
    summaryStats: {
        totalLeads: number;
...
const OUTCOME_COLORS = ['#0ea5e9', '#22c55e', '#f97316', '#8b5cf6', '#f43f5e', '#eab308', '#14b8a6'];
...
const ManagerReports: React.FC = () => {
    ...
    const fetchReportData = async () => {
        try {
            ...
            let url = `${API_BASE_URL}/api/dashboard/manager-reports`;
    ...
    return (
        <div className="h-full overflow-y-auto pr-1 -mr-1 custom-scrollbar space-y-6 sm:space-y-8 bg-[#f8fafc] pb-10">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in fade-in duration-500">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-extrabold text-[#0f1c2e] tracking-tight">Team Reports & Analytics</h1>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <p className="text-xs sm:text-sm text-slate-500">Deep dive into your team's lead pipeline, call performance, and conversions.</p>
                        {selectedBDExecutive && (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[0.65rem] font-bold bg-sky-50 text-[#0ea5e9] border border-sky-200 animate-in fade-in duration-200">
                                <Users size={10} />
                                {data.reportees.find(e => e._id === selectedBDExecutive)?.name || data.managerInfo?.name || 'Selected'}
                            </span>
                        )}
                    </div>
                </div>
            ...
            {/* Summary Cards */}
            {data.summaryStats && (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-5 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200/60 hover:shadow-md transition-all group">
                        ...
                        <p className="text-slate-500 text-[10px] font-medium uppercase tracking-wider">Team Leads</p>
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200/60 hover:shadow-md transition-all group">
                        ...
                        <p className="text-slate-500 text-[10px] font-medium uppercase tracking-wider">Team Members</p>
                        <h3 className="text-2xl font-bold text-[#0f1c2e] mt-1">{data.summaryStats.totalReportees}</h3>
                    </div>
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200/60 hover:shadow-md transition-all group">
                        <div className="flex justify-between items-start mb-3">
                            <div className="bg-sky-50 p-2 rounded-xl group-hover:bg-sky-100 transition-colors">
                                <PhoneCall className="text-sky-500" size={20} />
                            </div>
                        </div>
                        <p className="text-slate-500 text-[10px] font-medium uppercase tracking-wider">Total Calls</p>
                        <h3 className="text-2xl font-bold text-[#0f1c2e] mt-1">{data.summaryStats.totalCalls.toLocaleString()}</h3>
                    </div>
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200/60 hover:shadow-md transition-all group">
                        <div className="flex justify-between items-start mb-3">
                        ...
                        <p className="text-slate-500 text-[10px] font-medium uppercase tracking-wider">Proposals Sent</p>
                        <h3 className="text-2xl font-bold text-[#0f1c2e] mt-1">{data.summaryStats.totalProposalSent.toLocaleString()}</h3>
                    </div>
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200/60 hover:shadow-md transition-all group col-span-2 sm:col-span-1">
                        <div className="flex justify-between items-start mb-3">
                            <div className="bg-purple-50 p-2 rounded-xl group-hover:bg-purple-100 transition-colors">
                                <Award className="text-purple-500" size={20} />
                            </div>
                        </div>
                        <p className="text-slate-500 text-[10px] font-medium uppercase tracking-wider">Total Onboarded</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Team Calls Timeline (Area Chart) */}
                        ...
                        {data.monthlyTimeline.length > 0 ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={data.monthlyTimeline} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                    <defs>
                                        <linearGradient id="colorTeamCalls" x1="0" y1="0" x2="0" y2="1">
                                    ...
                                    <Area type="monotone" dataKey="calls" name="Calls Made" stroke="#0ea5e9" strokeWidth={3} fillOpacity={1} fill="url(#colorTeamCalls)" />
                                </AreaChart>
                            </ResponsiveContainer>
                        ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: '300ms' }}>
                    ...
                    <div className="p-6 flex-1 overflow-y-auto custom-scrollbar max-h-[300px]">
                                        ...
                                        <div key={index} className="group">
                                            <div className="flex items-center justify-between mb-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm font-bold text-slate-700 truncate max-w-[160px] group-hover:text-[#0ea5e9] transition-colors">
                                                        {industry.name}
                                                    </span>
                                                </div>
                                                <div className="flex items-center gap-2">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: '400ms' }}>
                <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/30">
                    <h3 className="font-extrabold text-[#0f1c2e] flex items-center gap-2">
                        <Award size={20} className="text-rose-500" />
                        Team Performance Matrix
                    </h3>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Sorted by Calls</span>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[700px]">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-100 text-[10px] sm:text-xs uppercase tracking-widest text-slate-400 font-extrabold">
                                ...
                                <th className="py-4 px-6 font-bold text-center">Won</th>
                        ...
                        <tbody className="text-sm font-medium text-slate-700 divide-y divide-slate-100">
                            {data.agentPerformance.length > 0 ? (
                                data.agentPerformance.map((agent, i) => (
                                    <tr key={i} className="hover:bg-slate-50/60 transition-colors group">
                                        ...
                                        <td className="py-4 px-6 text-center">
                                            <span className={`text-[10px] font-black px-2.5 py-1 rounded-lg uppercase tracking-wider ${agent.role === 'Manager' ? 'bg-purple-50 text-purple-600 border border-purple-100' : 'bg-sky-50 text-[#0ea5e9] border border-sky-100'}`}>
                                                {agent.role}
                                            </span>
                                        </td>
                                        <td className="py-4 px-6 text-center">
                                ...
                                <tr>
                                    <td colSpan={7} className="py-10 text-center text-slate-400 italic">No team performance data found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
            ...
            {data.reportees && data.reportees.length > 0 && (
                <div className="animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: '500ms' }}>
                    <div className="flex items-center gap-3 mb-5">
                        <div className="bg-gradient-to-br from-[#0ea5e9] to-[#0284c7] p-2.5 rounded-xl shadow-sm">
                            <UserCheck size={20} className="text-white" />
                        </div>
                        <div>
                            <h3 className="font-extrabold text-[#0f1c2e] text-lg">My Reportees</h3>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                {data.reportees.length} Active Team Member{data.reportees.length !== 1 ? 's' : ''}
                            </p>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {data.reportees.map((reportee, idx) => {
                            const agentData = data.agentPerformance.find(a => a.agentId === reportee._id);
                            return (
                                <div
                                    key={reportee._id}
                                    className="bg-white rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 overflow-hidden group"
                                    style={{ animationDelay: `${(idx + 6) * 80}ms` }}
                                >
                                    {/* Card header accent */}
                                    <div className="h-1.5 bg-gradient-to-r from-[#0ea5e9] via-[#3b82f6] to-[#8b5cf6]"></div>

                                    <div className="p-5">
                                        {/* Avatar & Name */}
                                        <div className="flex items-center gap-3.5 mb-4">
                                            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-slate-100 to-slate-50 border-2 border-slate-200 flex items-center justify-center text-lg font-black text-slate-500 group-hover:from-[#0ea5e9]/10 group-hover:to-[#0ea5e9]/5 group-hover:border-[#0ea5e9]/30 group-hover:text-[#0ea5e9] transition-all shadow-sm">
                                                {reportee.profile_photo ? (
                                                    <img src={`${API_BASE_URL}${reportee.profile_photo}`} alt={reportee.name} className="w-full h-full rounded-xl object-cover" />
                                                ) : (
                                                    reportee.name.charAt(0).toUpperCase()
                                                )}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <h4 className="text-sm font-extrabold text-[#0f1c2e] truncate">{reportee.name}</h4>
                                                <span className="text-[10px] font-black px-2 py-0.5 rounded-md bg-sky-50 text-[#0ea5e9] uppercase tracking-wider border border-sky-100">
                                                    {reportee.role}
                                                </span>
                                            </div>
                                            <div className={`w-2.5 h-2.5 rounded-full ${reportee.status === 'Active' ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]' : 'bg-slate-300'}`}></div>
                                        </div>
                                        ...
                                        <div className="space-y-2.5 mb-4">
                                            <div className="flex items-center gap-2 text-xs text-slate-500">
                                                ...
                                                <span className="truncate font-medium">{reportee.email}</span>
                                            </div>
                                            <div className="flex items-center gap-2 text-xs text-slate-500">
                                                <Phone size={12} className="text-slate-400 flex-shrink-0" />
                                                <span className="font-medium">{reportee.phone}</span>
                                            </div>
                                            {reportee.date_of_joining && (
                                                <div className="flex items-center gap-2 text-xs text-slate-500">
                                                    <Calendar size={12} className="text-slate-400 flex-shrink-0" />
                                                    <span className="font-medium">Joined {new Date(reportee.date_of_joining).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })}</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        {agentData && (
                                            <div className="grid grid-cols-3 gap-2 pt-3 border-t border-slate-100">
                                                <div className="text-center">
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Calls</p>
                                                    <p className="text-sm font-extrabold text-[#0f1c2e]">{agentData.calls}</p>
                                                </div>
                                                <div className="text-center border-x border-slate-100">
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Leads</p>
                                                    <p className="text-sm font-extrabold text-[#0f1c2e]">{agentData.leads}</p>
                                                </div>
                                                <div className="text-center">
                                                    <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider">Won</p>
                                                    <p className="text-sm font-extrabold text-emerald-600">{agentData.onboarded}</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
