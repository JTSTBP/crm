  ...
  const MetricCard: React.FC<{ 
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <p className="text-gray-300 text-sm font-semibold tracking-wide">{title}</p>
  ...
  const UserMetricCard: React.FC<{ user: any; rank: number }> = ({ user, rank }) => (
    <div 
      className={`glass rounded-xl p-4 border border-white/30 hover:shadow-lg transition-all duration-300 cursor-pointer hover:scale-[1.02] ${
        selectedUser === user.id ? 'ring-2 ring-blue-400' : ''
      }`}
    ...
    >
      ...
      <div className="grid grid-cols-4 gap-3 text-center">
        <div>
          <p className="text-lg font-semibold text-blue-400">{user.callsMade}</p>
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-purple-600 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center justify-between relative z-10">
          <div>
            <h1 className="text-3xl font-bold mb-2 tracking-tight">CRM Analytics Dashboard</h1>
            <p className="text-blue-100 text-lg">Comprehensive insights into sales performance and team metrics</p>
          </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="glass rounded-2xl p-6 border border-white/30 shadow-xl">
              <h2 className="text-xl font-bold text-white mb-6">Sales Pipeline</h2>
            ...
            {/* Conversion Overview */}
            <div className="glass rounded-2xl p-6 border border-white/30 shadow-xl">
              <h2 className="text-xl font-bold text-white mb-6">Conversion Overview</h2>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="glass rounded-2xl p-6 border border-white/30 shadow-xl">
              <h2 className="text-xl font-bold text-white mb-6">Industry Breakdown</h2>
              <div className="space-y-4">
                {overallMetrics.industryBreakdown.map((industry, index) => (
                  <div key={industry.industry} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="text-gray-400 text-sm">({industry.percentage}%)</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {activeTab === 'users' && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-white">Team Performance Leaderboard</h2>
            <div className="flex items-center space-x-4">
              <select
      ...
      {activeTab === 'activity' && (
        <div className="glass rounded-2xl p-6 border border-white/30 shadow-xl">
          <h2 className="text-xl font-bold text-white mb-6">Recent Activity Feed</h2>
            ...
            {activityFeed.map((activity) => (
              <div key={activity.id} className="flex items-start space-x-4 p-4 bg-white/5 rounded-xl border border-white/10">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                  ...
                  {activity.type === 'stage_change' && <TrendingUp className="w-5 h-5 text-white" />}
                  {activity.type === 'task' && <CheckSquare className="w-5 h-5 text-white" />}
                  {activity.type === 'lead_created' && <Building2 className="w-5 h-5 text-white" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-medium text-white">
                      <span className="text-blue-400">{activity.user}</span> {activity.action.toLowerCase()}
                    </p>
                    <p className="text-xs text-gray-400 flex-shrink-0 ml-2">
                      {format(new Date(activity.timestamp), 'MMM dd, HH:mm')}
                    </p>
                  </div>
                  <p className="text-sm text-gray-300 mt-1">{activity.leadName}</p>
                  {activity.details && (
