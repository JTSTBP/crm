...
const ManagerDashboard: React.FC = () => {
  ...
  const stats = {
    ...
    activeDeals: leads.filter(l => ['Contacted', 'Proposal Sent', 'Negotiation'].includes(l.stage)).length,
    won: leads.filter(l => l.stage === 'Won').length,
    revenue: leads.filter(l => l.stage === 'Won').reduce((sum, l) => sum + (l.value || 0), 0),
  ...
  const stageData = [
    ...
    { name: 'Negotiation', count: leads.filter(l => l.stage === 'Negotiation').length },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-500 to-pink-600 rounded-2xl p-6 text-white">
        ...
        <p className="text-purple-100">Monitor your team's progress and identify opportunities</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-xl p-6 border border-white/20">
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
            {stageData.filter(d => d.count > 0).map((stage, index) => (
              <div key={stage.name} className="flex items-center space-x-2">
                <div 
                  className="w-3 h-3 rounded-full" 
                  style={{ backgroundColor: pieColors[index % pieColors.length] }}
                />
                <span className="text-sm text-gray-600">{stage.name}: {stage.count}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-3 px-2 font-medium text-gray-600">Executive</th>
                <th className="text-center py-3 px-2 font-medium text-gray-600">Leads</th>
                <th className="text-center py-3 px-2 font-medium text-gray-600">Proposals</th>
                <th className="text-center py-3 px-2 font-medium text-gray-600">Won</th>
                <th className="text-center py-3 px-2 font-medium text-gray-600">Conversion</th>
                <th className="text-center py-3 px-2 font-medium text-gray-600">Revenue</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {[
                { name: 'Rahul Sharma', leads: 15, proposals: 8, won: 3, revenue: 450000 },
                { name: 'Priya Patel', leads: 12, proposals: 6, won: 4, revenue: 680000 },
                { name: 'Arjun Kumar', leads: 18, proposals: 10, won: 2, revenue: 320000 },
                { name: 'Sneha Singh', leads: 9, proposals: 5, won: 3, revenue: 540000 },
              ].map((exec) => (
                <tr key={exec.name} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="text-center py-3 px-2 text-gray-600">{exec.leads}</td>
                  <td className="text-center py-3 px-2 text-gray-600">{exec.proposals}</td>
                  <td className="text-center py-3 px-2 text-gray-600">{exec.won}</td>
                  <td className="text-center py-3 px-2">
                    <span className="text-emerald-600 font-medium">
                      {Math.round((exec.won / exec.leads) * 100)}%
                    </span>
                  </td>
