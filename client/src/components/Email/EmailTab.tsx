...
import React, { useState } from "react";
import { Send, Edit3, Search } from "lucide-react";
import { useEmails } from "../../hooks/useEmails";
import SentList from "./SentList";
import ComposeModal from "./ComposeModal";
import EmailDetailModal from "./EmailDetailModal";
...
const EmailTab: React.FC = () => {
  const { getUnreadCount } = useEmails();
  const [activeSubTab, setActiveSubTab] = useState<"sent">("sent");
  const [isComposeModalOpen, setIsComposeModalOpen] = useState(false);
  const [selectedEmail, setSelectedEmail] = useState<any>(null);
  const [emailType, setEmailType] = useState<"sent" | null>(null);
  const [searchTerm, setSearchTerm] = useState("");
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-300 mt-2 text-lg">
            Manage all your sent email communications in one place
          </p>
        </div>
        <button
          onClick={() => setIsComposeModalOpen(true)}
