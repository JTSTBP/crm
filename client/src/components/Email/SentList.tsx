...
const SentList: React.FC<SentListProps> = ({ searchTerm, onEmailSelect }) => {
  const { sentEmails, loading, deleteEmail, searchEmails } = useEmails()
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center space-x-4 mt-3 text-xs">
                      <span className="text-gray-500">
                        To: {email.to.length > 1 ? `${email.to[0]} +${email.to.length - 1} more` : email.to[0]}
                      </span>
                      {email.cc && email.cc.length > 0 && (
                        <span className="text-gray-500">CC: {email.cc.length}</span>
                      )}
                      {email.attachments.length > 0 && (
