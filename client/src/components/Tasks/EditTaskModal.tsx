...
const EditTaskModal: React.FC<EditTaskModalProps> = ({
  ...
  const { updateTask } = useLeadsContext();
  ...
  useEffect(() => {
    if (task) {
      ...
      hours = hours % 12 || 12;
      const dueTime = `${hours.toString().padStart(2, "0")}:${minutes
        .toString()
        .padStart(2, "0")} ${ampm}`;
      ...
      reset({
        ...
        due_date: dueDate,
        due_time: dueTime, // 👈 set formatted time
        type: task.type || "email", // 👈 set type
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {(profile?.role === "Admin" || profile?.role === "Manager") && (
            <div>
              ...
              <select
                {...register("user_id")}
                value={watch("user_id") || ""} // 👈 ensures controlled value
                onChange={(e) => setValue("user_id", e.target.value)} // 👈 update RHF state
                className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl 
             text-white focus:ring-2 focus:ring-blue-400 focus:border-blue-400 
