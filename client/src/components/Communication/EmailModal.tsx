...
const EmailModal: React.FC<EmailModalProps> = ({ isOpen, onClose, lead }) => {
  ...
  useEffect(() => {
    if (selectedTemplate) {
      // Use the first point of contact
      const firstContact = lead.points_of_contact?.[0] || {};
      const defaultValues: Record<string, string> = {
        company_name: lead.company_name || "",
        contact_name: firstContact.name || "", // <-- first contact name
        contact_email: firstContact.email || "", // <-- first contact email
        consultant_name: profile?.name || "",
  ...
  const generatePreview = () => {
    ...
    Object.entries(placeholderValues).forEach(([key, value]) => {
      ...
      const safeValue = value || ""; // fallback if empty
  ...
  const handleSendEmail = async () => {
    ...
    // Use first contact and company email
    const firstContact = lead.points_of_contact?.[0] || {};
    const toEmail = lead.company_email || firstContact.email || "";

    // Generate final subject and content with placeholders replaced
    const { subject: finalSubject, content: finalContent } = generatePreview();
const htmlContent = finalContent.replace(/\n/g, "<br>");
    try {
     await sendEmail({
       toEmail, // the actual email string
